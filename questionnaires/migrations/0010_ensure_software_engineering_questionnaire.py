...
def load_software_engineering_questionnaire(apps, schema_editor):
    """
    Load Software Engineering 360 Review questionnaire from fixture.
    ...
    if existing:
        print("✓ Software Engineering 360 Review template already exists")
        questionnaire = existing
    else:
        # Load fixture data
        fixture_path = Path(__file__).parent.parent / 'fixtures' / 'software_engineering_questionnaire.json'
        ...
        print("✓ Created Software Engineering 360 Review template")
    ...
    if cloned_count > 0:
        print(f"✓ Cloned Software Engineering 360 Review to {cloned_count} organization(s)")
...
def reverse_software_engineering_questionnaire(apps, schema_editor):
    """Remove Software Engineering 360 Review questionnaire"""
    Questionnaire = apps.get_model('questionnaires', 'Questionnaire')
    ...
    if deleted_count > 0:
        print(f"Deleted {deleted_count} 'Software Engineering 360 Review' questionnaire(s)")


class Migration(migrations.Migration):

    dependencies = [
        ('questionnaires', '0009_add_uuid_field'),
        ('core', '0001_initial'),  # Need Organization model
    ]

    operations = [
        migrations.RunPython(
            load_software_engineering_questionnaire,
            reverse_software_engineering_questionnaire
        ),
    ]
